...
int iterCount, feats_down_size, NUM_MAX_ITERATIONS, laserCloudValidNum, effct_feat_num,
    time_log_counter, publish_count = 0;
...
void get_leaf_nums(size_t& voxel_map_size, OctoTree* voxel) {
  if (voxel->layer_ == voxel->max_layer_) {
    return;
  }
  for (size_t leafnum = 0; leafnum < 8; leafnum++) {
    if (voxel->leaves_[leafnum] != nullptr) {
      voxel_map_size++;
      get_leaf_nums(voxel_map_size, voxel->leaves_[leafnum]);
    }
  }
}

size_t get_voxel_map_size(std::unordered_map<VOXEL_LOC, OctoTree*>& voxel_map) {
  size_t voxel_map_size = 0;
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
    voxel_map_size++;
    get_leaf_nums(voxel_map_size, iter->second);
  }
  return voxel_map_size;
}

void get_leaf_point_size(size_t& voxel_map_point_size, OctoTree* voxel) {
  voxel_map_point_size += (voxel->temp_points_.size() + voxel->new_points_.size());
  for (size_t leafnum = 0; leafnum < 8; leafnum++) {
    if (voxel->leaves_[leafnum] != nullptr) {
      get_leaf_point_size(voxel_map_point_size, voxel->leaves_[leafnum]);
    }
  }
}

size_t get_voxel_map_point_size(std::unordered_map<VOXEL_LOC, OctoTree*>& voxel_map) {
  size_t voxel_map_point_size = 0;
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter) {
    get_leaf_point_size(voxel_map_point_size, iter->second);
  }
  return voxel_map_point_size;
}
...
int main(int argc, char** argv) {
  ...
  // Enable voxel merging
  bool enable_voxel_merging = false;
  bool pub_merged_voxel = false;
  double merge_theta_thresh = 0.05;
  double merge_dist_thresh = 0.05;
  double merge_cov_min_eigen_val_thresh = 0.002;
  double merge_x_coord_diff_thresh = 5.0;
  double merge_y_coord_diff_thresh = 5.0;
  ...
  // vocel merging params
  nh.param<bool>("voxel_merging/enable_voxel_merging", enable_voxel_merging, false);
  nh.param<double>("voxel_merging/merge_theta_thresh", merge_theta_thresh, 0.05);
  nh.param<double>("voxel_merging/merge_dist_thresh", merge_dist_thresh, 0.05);
  nh.param<double>("voxel_merging/merge_cov_min_eigen_val_thresh",
                   merge_cov_min_eigen_val_thresh,
                   0.001);
  nh.param<double>("voxel_merging/merge_x_coord_diff_thresh", merge_x_coord_diff_thresh, 5.0);
  nh.param<double>("voxel_merging/merge_y_coord_diff_thresh", merge_y_coord_diff_thresh, 5.0);
  ...
  nh.param<bool>("visualization/pub_merged_voxel", pub_merged_voxel, false);
  ...
  ros::Publisher pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered",
                                                                               100);
  ...
  ros::Publisher merged_voxel_publisher = nh.advertise<visualization_msgs::MarkerArray>(
      "/merged_voxels",
      10000);
  ...
  // Plane hash map
  std::unordered_map<PlaneDesc, UnionPlane> plane_hash_map;
  std::unordered_map<int, std::unordered_set<NodeLoc>> merge_node_table;
  ...
  while (status) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (flg_EKF_inited && !init_map) {
        ...
        buildVoxelMap(enable_voxel_merging,
                      pv_list,
                      max_voxel_size,
                      max_layer,
                      layer_size,
      ...
      for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++) {
        ...
        {
          BuildResidualListOMP(voxel_map,
                               max_voxel_size,
                               3.0,
                               max_layer,
                               pv_list,
          ...
          scan_match_time_end = std::chrono::high_resolution_clock::now();
        ...
        for (int i = 0; i < effct_feat_num; i++) {
          ...
          laserCloudOri->points[i].curvature = sqrt(sigma_l +
                                                    norm_vec.transpose() * cov * norm_vec);

          /*** calculate the Measuremnt Jacobian matrix H ***/
          V3D A(point_crossmat * state.rot_end.transpose() * norm_vec);
          Hsub.row(i) << VEC_FROM_ARRAY(A), norm_p.x, norm_p.y, norm_p.z;
          Hsub_T_R_inv.col(i) << A[0] * R_inv(i), A[1] * R_inv(i), A[2] * R_inv(i),
              norm_p.x * R_inv(i), norm_p.y * R_inv(i), norm_p.z * R_inv(i);
        ...
        if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == NUM_MAX_ITERATIONS - 1))) {
          if (flg_EKF_inited) {
            ...
            geoQuat = tf::createQuaternionMsgFromRollPitchYaw(euler_cur(0),
                                                              euler_cur(1),
                                                              euler_cur(2));
        ...
        solve_time += std::chrono::duration_cast<std::chrono::duration<double>>(t_solve_end -
                                                                                t_solve_start)
                          .count() *
                      1000;
      ...
      updateVoxelMap(enable_voxel_merging,
                     pv_list,
                     max_voxel_size,
                     max_layer,
                     layer_size,
      ...
      /******* Publish function:  *******/
      {
        ...
        if (pub_merged_voxel && (frame_num % 30 == 0)) {
          PublishMergedVoxelPlane(voxel_map,
                                  merge_node_table,
                                  merged_voxel_publisher,
                                  max_voxel_size);
        }
      }
      ...
      down_sample_time_mean = down_sample_time_mean * (frame_num - 1) / frame_num +
                              (t_downsample) / frame_num;
      calc_cov_time_mean = calc_cov_time_mean * (frame_num - 1) / frame_num +
                           (calc_point_cov_time) / frame_num;
      scan_match_time_mean = scan_match_time_mean * (frame_num - 1) / frame_num +
