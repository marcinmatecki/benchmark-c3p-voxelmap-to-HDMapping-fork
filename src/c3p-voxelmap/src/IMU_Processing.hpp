...
/// *************Preconfiguration
#define INITIAL_MEA_NOISE (0.0001)
#define NITIAL_BIAS_NOISE (0.00001)
...
class ImuProcess {
 ...
 private:
  ...
  void only_propag(const MeasureGroup& meas,
...
void ImuProcess::UndistortPcl(const MeasureGroup& meas,
  ...
  for (auto it_imu = v_imu.begin(); it_imu != v_imu.end() - 1; it_imu++) {
    ...
    if (tail->header.stamp.toSec() < time_last_scan_)
      continue;
    ...
    if (head->header.stamp.toSec() < time_last_scan_) {
      dt = tail->header.stamp.toSec() - time_last_scan_;
    } else {
...
void ImuProcess::only_propag(const MeasureGroup& meas,
                             StatesGroup& state_inout,
                             PointCloudXYZI::Ptr& pcl_out) {
  const double& pcl_beg_time = meas.lidar_beg_time;
...
void ImuProcess::Process(const MeasureGroup& meas,
  ...
  if (imu_need_init_ && imu_en) {
    ...
    if (init_iter_num > MAX_INI_COUNT) {
      ...
      ROS_INFO("Gravity: %.4f %.4f %.4f %.4f; state.bias_g: %.8f "
               "%.8f %.8f; acc covarience: %.8f %.8f %.8f; gry covarience: "
               "%.8f %.8f %.8f",
               stat.gravity[0],
               stat.gravity[1],
               stat.gravity[2],
